...
export async function addCardToBoard({ board_id, card_id }) {
  const sql = `
    ...
    RETURNING board_id, card_id, x_pos, y_pos;
  `;
  const { rows } = await query(sql, [board_id, card_id]);
  return rows[0];
}

export async function updateBoardCardPosition({ board_id, card_id, x_pos, y_pos }) {
  const sql = `
    ...
    RETURNING board_id, card_id, x_pos, y_pos;
  `;
  const { rows } = await query(sql, [board_id, card_id, x_pos, y_pos]);
  return rows[0];
}
